...
-- Creates optimized database functions for 50k user scaling
-- =====================================================
...
-- =====================================================
-- Core Dashboard Functions with CTEs
-- =====================================================

-- Get comprehensive dashboard data in single query
CREATE OR REPLACE FUNCTION get_dashboard_data(p_business_id UUID)
RETURNS TABLE(
...
BEGIN
    ...
    recent_transactions AS (
        ...
        FROM (
            SELECT id, amount, customer_name, transaction_date, status
            FROM transactions
            WHERE business_id = p_business_id
    ...
    outstanding_credit AS (
        SELECT COALESCE(SUM(amount - paid_amount), 0) as credit
        FROM credit
        WHERE business_id = p_business_id
    ...
    SELECT 
        ...
        te.expenses,
        oc.credit
    FROM today_revenue tr, month_revenue mr, pending_appointments pa, 
         recent_transactions rt, total_expenses te, outstanding_credit oc;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Get business summary with all metrics
