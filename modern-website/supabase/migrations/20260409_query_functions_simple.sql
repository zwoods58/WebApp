-- =====================================================
-- Phase 2: Simple Query Optimization Functions
-- Functions that work with actual table structure
-- =====================================================
...
-- =====================================================
-- Business Summary Function (works with actual schema)
-- =====================================================
...
-- =====================================================
-- Monthly Report Function (works with actual schema)
-- =====================================================
...
-- =====================================================
-- Realtime-Specific Functions (for Phase 0 Integration)
-- =====================================================

-- Get recent transactions for realtime updates (no status filter)
CREATE OR REPLACE FUNCTION get_recent_realtime_transactions(p_business_id UUID, p_limit INTEGER DEFAULT 100)
RETURNS TABLE(
    id UUID,
    business_id UUID,
    amount DECIMAL,
...
-- Get recent expenses for realtime updates (no status filter)
CREATE OR REPLACE FUNCTION get_recent_realtime_expenses(p_business_id UUID, p_limit INTEGER DEFAULT 100)
RETURNS TABLE(
    id UUID,
    business_id UUID,
    amount DECIMAL,
...
GRANT EXECUTE ON FUNCTION get_recent_realtime_transactions(UUID, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION get_recent_realtime_expenses(UUID, INTEGER) TO authenticated;
...
GRANT EXECUTE ON FUNCTION get_recent_realtime_transactions(UUID, INTEGER) TO service_role;
GRANT EXECUTE ON FUNCTION get_recent_realtime_expenses(UUID, INTEGER) TO service_role;
...
-- Business ID indexes for faster queries
...
CREATE INDEX IF NOT EXISTS idx_credit_business_id_status 
ON credit(business_id, status);

CREATE INDEX IF NOT EXISTS idx_appointments_business_id_date 
