-- =====================================================
-- Kyshi Payment Integration - Clean Schema (No Workaround)
-- Migration: Creates Kyshi tables without workaround fields
-- Date: 2026-04-12
-- Purpose: Clean Kyshi integration without amount multiplication
-- =====================================================

-- =====================================================
-- 1. KYSHI PLANS TABLE
-- Stores subscription plans (clean version)
-- =====================================================
...
COMMENT ON TABLE kyshi_plans IS 'Subscription plans for Kyshi payment integration';
COMMENT ON COLUMN kyshi_plans.amount IS 'Plan amount in local currency';
COMMENT ON COLUMN kyshi_plans.kyshi_plan_code IS 'Plan code from Kyshi API';
...
-- Stores subscription records (clean version)
-- =====================================================
...
COMMENT ON TABLE kyshi_subscriptions IS 'Customer subscriptions for Kyshi payment integration';

-- =====================================================
-- 4. KYSHI TRANSACTIONS TABLE
-- Stores payment transactions (clean version)
-- =====================================================
...
COMMENT ON TABLE kyshi_transactions IS 'Payment transactions for Kyshi integration';
COMMENT ON COLUMN kyshi_transactions.amount IS 'Transaction amount in local currency';
...
-- =====================================================
-- 9. SAMPLE CLEAN PLANS (for testing)
-- These use real amounts without any multiplication
-- =====================================================

-- Kenya: 200 KES
INSERT INTO kyshi_plans (country_code, name, amount, currency, interval)
VALUES (
  'KE',
  'Beezee Weekly Kenya',
  200,
...
VALUES (
  'NG',
  'Beezee Weekly Nigeria',
  500,
  'NGN',
  'weekly'
) ON CONFLICT (kyshi_plan_code) DO NOTHING;

-- Côte d'Ivoire: 1,000 XOF
INSERT INTO kyshi_plans (country_code, name, amount, currency, interval)
VALUES (
  'CI',
  'Beezee Weekly Côte d''Ivoire',
  1000,
  'XOF',
  'weekly'
) ON CONFLICT (kyshi_plan_code) DO NOTHING;

COMMENT ON SCRIPT IS 'Clean Kyshi payment integration schema without amount workaround';
