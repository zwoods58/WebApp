...
self.addEventListener('online', () => {
  isOffline = false;
  console.log('[SW] ðŸŒ Back online');
});

self.addEventListener('offline', () => {
  isOffline = true;
  console.log('[SW] ðŸ“´ Gone offline - switching to cache-only mode');
});
...
self.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'OFFLINE_STATUS') {
    ...
    console.log('[SW] ðŸ“Š Offline status updated from main thread:', isOffline ? 'OFFLINE' : 'ONLINE');
  }
});
...
if (IS_DEV) {
  console.log('[SW] ðŸ”§ Development mode detected - aggressive cache invalidation enabled');
}
...
self.addEventListener('install', (event) => {
  console.log(`[SW] ðŸš€ Installing service worker ${CACHE_VERSION}...`);
  ...
  event.waitUntil(
    (async () => {
      ...
      console.log('[SW] ðŸ§¹ Clearing old caches:', oldCaches);
        ...
        for (const route of PUBLIC_ROUTES) {
          try {
            ...
            if (response.ok) {
              ...
              failedCount++;
              console.warn(`[SW] âš ï¸ Failed to cache ${route}: ${response.status}`);
            }
          } catch (err) {
            failedCount++;
            console.warn(`[SW] âš ï¸ Error caching ${route}:`, err.message);
          }
        }
        
        console.log(`[SW] âœ… Cached ${cachedCount}/${PUBLIC_ROUTES.length} assets (${failedCount} failed)`);
      } catch (error) {
        console.warn('[SW] âš ï¸ Cache operation error:', error);
      }
      ...
      console.log(`[SW] ðŸ“¢ Notified ${clients.length} clients of update availability`);
    })()
  );
});
...
self.addEventListener('activate', (event) => {
  console.log(`[SW] ðŸ”„ Activating ${CACHE_VERSION}...`);
  ...
  event.waitUntil(
    (async () => {
      ...
      await clients.claim();
      console.log(`[SW] âœ… Service worker ${CACHE_VERSION} activated and claimed clients`);
...
self.addEventListener('message', (event) => {
  ...
  console.log('[SW] ðŸ“¨ Message source:', event.source);
  console.log('[SW] ðŸ“¨ Message origin:', event.origin);
  
  const { type, country, industry } = event.data;
  
  // Handle user route caching after login
  if (type === 'CACHE_USER_ROUTES') {
    console.log('[SW] ðŸ“¦ Received CACHE_USER_ROUTES message!');
    console.log('[SW] ðŸ“¦ User routes data:', { country, industry });
    console.log('[SW] ðŸ“¦ Current stored routes:', { userCountry, userIndustry });
    
    // Update stored user info
    userCountry = country;
    userIndustry = industry;
    
    console.log('[SW] ðŸš€ Starting cacheUserRoutes...');
    
    // Cache user's routes in background (don't block)
    event.waitUntil(
      cacheUserRoutes(country, industry).then(() => {
        console.log('[SW] âœ… cacheUserRoutes completed successfully!');
      }).catch(error => {
        console.error('[SW] âŒ cacheUserRoutes failed:', error);
      })
    );
  ...
  if (type === 'CHECK_UPDATE') {
    console.log('[SW] ðŸ” CHECK_UPDATE received - checking for new version...');
    event.waitUntil(
      (async () => {
        const registration = await self.registration;
        await registration.update();
        console.log('[SW] âœ… Update check completed');
      })()
    );
    return;
  }
  ...
  if (type === 'SKIP_WAITING') {
    console.log('[SW] ðŸ”„ SKIP_WAITING received, activating new version...');
    ...
    self.clients.matchAll().then(clients => {
      ...
      console.log('[SW] âœ… Notified', clients.length, 'clients of activation');
    });
  }
...
async function fetchWithTimeout(request, timeout = 3000) {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      console.warn('[SW] â±ï¸ Request timeout after', timeout, 'ms:', request.url);
...
async function fetchFromCacheOnly(request) {
    ...
    if (url.pathname.endsWith('.js')) {
      try {
        ...
        if (!text || text.length < 10 || text.startsWith('<!DOCTYPE') || text.includes('503 Service Unavailable')) {
          console.log('[SW] âš ï¸ Corrupted JS cache detected, removing:', request.url);
          const cache = await caches.open(STATIC_CACHE);
          await cache.delete(request);
          return null;
        }
      } catch (err) {
        console.log('[SW] âš ï¸ Cache validation failed, removing:', request.url);
        const cache = await caches.open(STATIC_CACHE);
        await cache.delete(request);
        return null;
      }
  ...
  console.log('[SW] âŒ Cache miss:', request.url);
  return null;
}
...
async function cacheUserRoutes(country, industry) {
  console.log(`[SW] ðŸ“¦ cacheUserRoutes called with:`, { country, industry });
  console.log(`[SW] ðŸ“¦ Starting to cache routes for authenticated user...`);
  
  // Wait 2 seconds for session to fully establish after login
  console.log(`[SW] â³ Waiting 2 seconds for session to establish...`);
  await new Promise(resolve => setTimeout(resolve, 2000));
  console.log(`[SW] â° Session wait complete, starting caching...`);
  ...
  console.log(`[SW] ðŸ“¦ Routes to cache:`, routesToCache);
  console.log(`[SW] ðŸ“¦ Starting to cache ${routesToCache.length} routes + chunks for: ${country}/${industry}`);
  ...
  for (const route of routesToCache) {
    console.log(`[SW] ðŸ”„ Processing route ${cached + failed + 1}/${routesToCache.length}: ${route}`);
    
    try {
      // Fetch HTML page with cache control
      console.log(`[SW] ðŸ“¡ Fetching route: ${route}`);
      ...
      console.log(`[SW] ðŸ“Š Response for ${route}:`, {
        ok: htmlResponse.ok,
      ...
      if (!htmlResponse.ok || 
        ...
        failed++;
        console.warn(`[SW] âŒ Skipped (redirect or error): ${route}`, {
          ok: htmlResponse.ok,
          status: htmlResponse.status,
          url: htmlResponse.url,
      ...
      cached++;
      console.log(`[SW] âœ… Cached page (${cached}/${routesToCache.length}): ${route}`);
      
      // Extract and cache chunks
      const htmlText = await htmlResponse.text();
      const chunks = await extractChunksFromHTML(htmlText, self.location.origin);
      
      console.log(`[SW] ðŸ“¦ Found ${chunks.length} chunks for ${route}`);
      ...
      for (const chunkUrl of chunks) {
        try {
          ...
          if (chunkResponse.ok) {
            ...
            console.log(`[SW] âœ… Cached chunk: ${new URL(chunkUrl).pathname}`);
          }
        } catch (chunkError) {
          console.warn(`[SW] âš ï¸ Failed to cache chunk: ${chunkUrl}`, chunkError.message);
        }
      }
      ...
      failed++;
      console.warn(`[SW] âŒ Error caching ${route}:`, err.message);
    }
  }
  
  console.log(`[SW] ðŸŽ‰ Caching complete! âœ… ${cached} pages, ${totalChunks} chunks, âŒ ${failed} failed`);
  ...
  console.log(`[SW] ðŸ’¾ Total cache size: ${sizeMB} MB`);
  
  if (totalSize > 50 * 1024 * 1024) { // 50MB
    console.warn(`[SW] âš ï¸ Cache size exceeds 50MB! Consider cleanup.`);
  }
}
...
async function cacheAllDiscoveredChunks() {
  console.log('[SW] ðŸ“¦ Caching all discovered chunks...');
  ...
  try {
        ...
        for (const key of matchingKeys) {
          try {
            ...
            if (response && response.ok) {
              chunksCached++;
              console.log(`[SW] âœ… Verified cached chunk: ${key.url}`);
            }
          } catch (err) {
            console.warn(`[SW] âš ï¸ Failed to verify chunk: ${key.url}`);
          }
        }
      } catch (err) {
    ...
    console.log(`[SW] âœ… Verified ${chunksCached} chunks in cache`);
    
  } catch (error) {
    console.warn('[SW] âš ï¸ Error in cacheAllDiscoveredChunks:', error);
  }
}
...
async function cacheGoogleFonts() {
  console.log('[SW] ðŸ“¦ Caching Google Fonts...');
  const staticCache = await caches.open(STATIC_CACHE);
  let fontsCached = 0;
  ...
  for (const fontUrl of GOOGLE_FONTS) {
    try {
      ...
      if (response.ok) {
        ...
        fontsCached++;
        console.log(`[SW] âœ… Cached Google Font: ${fontUrl}`);
      }
    } catch (err) {
      console.warn(`[SW] âš ï¸ Failed to cache Google Font: ${fontUrl}`);
    }
  }
  
  console.log(`[SW] âœ… Cached ${fontsCached}/${GOOGLE_FONTS.length} Google Fonts`);
}
  ...
  try {
    ...
    for (const asset of assets) {
      try {
        ...
        console.warn(`[SW] âš ï¸ Failed to cache asset: ${asset}`);
      }
    }
    
    console.log(`[SW] âœ… Cached ${assetsCached}/${assets.length} static assets`);
  } catch (err) {
...
self.addEventListener('fetch', (event) => {
      ...
      (async () => {
        ...
        try {
          if (isActuallyOffline) {
            ...
            console.log('[SW] ðŸ“´ RSC request offline, checking cache:', url.pathname);
            
            // Try to find cached RSC response first
            const cachedRSC = await caches.match(request);
            if (cachedRSC) {
              console.log('[SW] âœ… Serving cached RSC response:', url.pathname);
              return cachedRSC;
            }
            ...
            for (const key of cacheKeys) {
              ...
              if (cached) {
                console.log('[SW] âœ… Serving cached HTML for RSC request:', key);
                return cached;
              }
            }
            
            // No cached RSC - return lightweight empty response (not 503!)
            console.log('[SW] ðŸ“´ Returning empty RSC response (offline):', url.pathname);
            return new Response('[]', {
              status: 200,
              headers: {
          ...
          console.log('[SW] ðŸŒ Fetching and caching RSC request:', url.pathname);
          try {
            const response = await fetch(request);
            if (response.ok) {
              const cache = await caches.open(PAGE_CACHE);
              cache.put(request, response.clone());
              console.log('[SW] ðŸ’¾ Cached RSC response:', url.pathname);
            }
            return response;
          } catch (error) {
  ...
  if (request.destination === 'document' || 
    ...
    event.respondWith(
      (async () => {
        ...
        console.log('[SW] ðŸ“Š Offline check:', {
        ...
        if (isActuallyOffline) {
          ...
          console.log('[SW] ðŸ” Trying cache keys for offline page:', { pathname: url.pathname, cacheKeys });
          
          // Try each cache key variation in current cache
          for (const key of cacheKeys) {
            const cached = await caches.match(key);
            if (cached) {
              console.log('[SW] âœ… Found cached page with key:', key);
              return cached;
            }
          }
          
          // Try cross-cache search (older versions) as fallback
          console.log('[SW] ðŸ” Trying cross-cache search for offline page...');
            ...
            if (cacheName.includes('beezee-pages') || cacheName.includes('beezee-static')) {
              try {
                ...
                for (const key of cacheKeys) {
                  ...
                  if (cached) {
                    console.log('[SW] âœ… Found in cross-cache:', cacheName, 'with key:', key);
                    return cached;
                  }
                }
              } catch (err) {
                console.warn('[SW] âš ï¸ Error accessing cache:', cacheName, err.message);
              }
            }
          }
          
          // Ultimate fallback - offline.html
          console.log('[SW] âŒ No cached page found, serving offline.html');
          const offlinePage = await caches.match(BASE_PATH + '/offline.html');
          if (offlinePage) {
            console.log('[SW] âœ… Serving offline.html');
            return offlinePage;
          }
          
          // Last resort - generated offline page
          console.log('[SW] âŒ No offline.html found, generating fallback page');
        ...
        if (cached) {
          console.log('[SW] âœ… Serving from cache:', cacheKey);
          return cached;
        }
        
        try {
          // Use fetchWithTimeout as fallback for unreliable navigator.onLine
          console.log('[SW] ðŸ“¡ Fetching from network:', cacheKey);
          ...
          console.log('[SW] ðŸ“´ Network failed, trying cache fallback:', cacheKey);
          const cached = await caches.match(cacheKey);
          if (cached) {
            console.log('[SW] âœ… Serving from cache (network fallback):', cacheKey);
            return cached;
          }
          
          // No cache available - serve offline.html
          console.log('[SW] âŒ No cache available, serving offline.html');
          const offlinePage = await caches.match(BASE_PATH + '/offline.html');
    ...
    event.respondWith(
      (async () => {
        ...
        if (isOffline || !navigator.onLine) {
          console.log('[SW] ðŸ“µ Offline mode - serving static asset:', url.pathname);
          
          const cached = await fetchFromCacheOnly(request);
          if (cached) {
            console.log('[SW] âœ… Serving static from cache (offline):', url.pathname);
            return cached;
          }
          ...
          console.log('[SW] âŒ Missing static asset offline:', url.pathname);
        ...
        const cached = await caches.match(request);
        if (cached) {
          console.log('[SW] âœ… Serving static from cache:', url.pathname);
          return cached;
        }
        
        try {
          // Use fetchWithTimeout as fallback for unreliable navigator.onLine
          console.log('[SW] ðŸ“¡ Fetching static asset:', url.pathname);
          ...
          console.log('[SW] âŒ Network failed/timeout for asset:', url.pathname);
          if (url.pathname.endsWith('.js')) {
            return new Response(
              `console.warn("Offline: Chunk not available - ${url.pathname}");`,
              { headers: { 'Content-Type': 'application/javascript' } }
  ...
  // ============================================================
  // âœ… UPDATED: API calls - ONLY cache internal APIs, NOT Supabase
  ...
  if (url.pathname.startsWith('/api/')) {
    event.respondWith(
      (async () => {
        ...
        if (isOffline || !navigator.onLine) {
          console.log('[SW] ðŸ“µ Offline mode - serving cached API data:', url.pathname);
          
          // Simple cache lookup
          const cached = await fetchFromCacheOnly(request);
          if (cached) {
            console.log('[SW] âœ… Serving cached API response:', url.pathname);
            return cached;
          }
          ...
          console.log('[SW] âŒ No cached API data found for:', url.pathname);
        ...
        const fetchPromise = fetchWithTimeout(request, 5000).then(async (networkResponse) => {
          if (networkResponse.ok) {
            ...
            console.log('[SW] âœ… Updated cached API:', url.pathname);
          }
          return networkResponse;
        }).catch((error) => {
          console.warn('[SW] âš ï¸ Background API fetch failed:', url.pathname);
          return null;
        });
        
        // Return cached instantly if available
        if (cached) {
          console.log('[SW] âœ… Serving cached API (background update):', url.pathname);
        ...
        console.log('[SW] ðŸ“¡ Fetching API from network:', url.pathname);
  ...
  // ============================================================
  // âœ… UPDATED: Supabase calls - NEVER CACHE (let TanStack Query handle it)
  // ============================================================
  if (url.hostname.includes('supabase.co')) {
    event.respondWith(
      (async () => {
        ...
        if (isActuallyOffline) {
          console.log('[SW] ðŸ“´ OFFLINE MODE - Blocking Supabase request, TanStack Query will use IndexedDB:', url.pathname);
          return new Response(
        ...
        console.log('[SW] ðŸŒ Online - Passing through Supabase request:', url.pathname);
        try {
          const response = await fetchWithTimeout(request, 10000);
          return response;
        } catch (error) {
          console.log('[SW] âš ï¸ Supabase fetch failed, treating as offline');
          return new Response(
