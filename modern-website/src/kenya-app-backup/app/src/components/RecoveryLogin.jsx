...
export default function RecoveryLogin({ phoneNumber, onSuccess, onBack }) {
    ...
    const handleRequestRecovery = async () => {
        ...
        try {
            ...
            const res = await AuthService.requestRecovery(localPhone, activeCountry?.code || 'KE');
            if (res.success) {
    ...
    return (
        <div className="bg-white rounded-3xl shadow-xl p-8 w-full">
            <div className="text-center mb-8">
                ...
                <p className="text-gray-600 text-sm">Securely reset your access</p>
            </div>
            ...
            {step === 1 && (
                <div className="space-y-6">
                    ...
                    <button onClick={handleRequestRecovery} disabled={loading} className="auth-cta-button bg-orange-600 hover:bg-orange-700">
            ...
            {step === 2 && (
                <div className="space-y-6">
                    ...
                    <div className="auth-form-field">
                        <label className="auth-form-label text-center block">New 6-Digit PIN</label>
                        <div className="flex justify-center gap-2">
                            {newPin.map((d, i) => (
                                <input key={i} id={`recover-pin-${i}`} type="password" value={d} onChange={(e) => handlePinInput(i, e.target.value)} className="w-10 h-12 border-2 rounded-lg text-center" maxLength={1} />
                            ))}
                        </div>
                    </div>

                    <div className="auth-form-field">
                        <label className="auth-form-label text-center block">Confirm New PIN</label>
                        <div className="flex justify-center gap-2">
                            {confirmPin.map((d, i) => (
                                <input key={i} id={`recover-confirm-${i}`} type="password" value={d} onChange={(e) => handlePinInput(i, e.target.value, true)} className="w-10 h-12 border-2 rounded-lg text-center" maxLength={1} />
                            ))}
                        </div>
                    </div>

                    <button onClick={handleResetSubmit} disabled={loading} className="auth-cta-button">
