...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[Dashboard] Error loading transactions:', error.message || error);
        throw error;
      }
      ...
      try {
        ...
        console.error('[Dashboard] Error loading offline transactions:', error.message || error);
      }
  ...
  const handleBookingSubmit = async (bookingData) => {
    try {
      ...
      console.log('[Dashboard] Saving booking:', bookingData);

      // Save booking to database
      const { data, error } = await supabase
        .from('bookings')
        .insert({ ...bookingData, user_id: finalUserId })
        .select()
        .single();
      ...
      console.error('[Dashboard] Error saving booking:', error);
      toast.error('Failed to save booking');
      throw error; // Re-throw so AddBookingModal can show error
    }
  };
