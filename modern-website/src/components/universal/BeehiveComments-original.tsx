...
// Helper function to get current business ID from UnifiedAuth
  ...
  try {
    ...
    if (businessAuth) {
      try {
        ...
        if (businessId) {
          console.log('Using business ID in comments:', businessId);
          return businessId;
        }
      } catch (e) {
...
export default function BeehiveComments({ requestId, onCommentAdded }: BeehiveCommentsProps) {
  ...
  // Get current business ID from UnifiedAuth - simple and reliable
  const getCurrentBusinessId = (): string | null => {
    const businessId = business?.id || user?.id || null;
    console.log('Getting business ID from UnifiedAuth:', { businessId, hasBusiness: !!business, hasUser: !!user });
    return businessId;
  };
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!businessId) throw new Error('Business not authenticated');

      console.log('💬 Adding comment:', { requestId, businessId, comment: newComment.trim() });
