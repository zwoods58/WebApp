...
export async function getCoachingContext(userId) {
  try {
    // Get user info with enhanced business details
    ...
    const { activeCountry } = useCountryStore.getState();
    const currencySymbol = activeCountry?.currency?.symbol || '₦';
    const countryName = activeCountry?.name || 'Nigeria';

    return {
      user_name: userData?.phone_number?.slice(-4) || 'there',
      business_type: userData?.business_type || 'informal business',
      location: userData?.location || countryName,
      registration_status: userData?.registration_status || 'not registered',
      business_age_months: userData?.business_age_months || 0,
...
export async function checkForProactiveInsights(userId) {
  try {
    ...
    const { activeCountry } = useCountryStore.getState();
    const symbol = activeCountry?.currency?.symbol || '₦';
    ...
    if (today.getDay() === 1) { // Monday
      insights.push({
        type: 'weekly_summary',
        message: `Your week in numbers: ${symbol}${context.current_month_profit.toFixed(0)} profit this month from ${context.transaction_count} transactions. ${context.trend === 'growing' ? 'Things are looking up! 📈' :
    ...
    if (context.current_month_profit < 0) {
      insights.push({
        type: 'warning',
        message: `⚠️ Your expenses (${symbol}${context.total_expenses.toFixed(0)}) were higher than your income (${symbol}${context.total_income.toFixed(0)}) this month. Want to look at where the money went?`,
      });
    }
...
export function formatContextForPrompt(context, history = []) {
  ...
  const recentTxText = context.recent_transactions.slice(0, 5)
    .map(t => `${t.date}: ${t.type === 'income' ? '+' : '-'}${symbol}${t.amount} (${t.category}) - ${t.description}`)
    .join('\n');
...
- Average daily income: ${symbol}${context.avg_daily_income.toFixed(2)}
- Average daily expenses: ${symbol}${context.avg_daily_expenses.toFixed(2)}
...
- Current month profit: ${symbol}${context.current_month_profit.toFixed(2)}
${inventoryText}
${appInfo}
