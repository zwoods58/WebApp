        ...
        {loading && notifications.length === 0 ? (
          ...
          <div className="space-y-3">
            {notifications.map((n) => (
              <div
                key={n.id}
                className={`notification-card relative bg-white p-4 rounded-2xl shadow-sm border-l-4 transition-all hover:shadow-md ${n.read ? 'border-gray-200 opacity-80' : 'border-primary ring-1 ring-primary/10'
                  }`}
                onClick={() => !n.read && markAsRead(n.id)}
              >
                {!n.read && (
                  <span className="absolute -top-1 -right-1 w-3 h-3 bg-primary rounded-full border-2 border-white shadow-sm animate-pulse" />
                )}
                ...
                <div className="flex items-center justify-between">
                  ...
                  {n.action_url && (
                    <button
                      ...
                      className="text-xs font-bold py-1.5 px-4 bg-primary text-white rounded-full shadow-sm hover:bg-primary-dark transition-colors flex items-center gap-1.5"
                    >
