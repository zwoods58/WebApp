          ...
          actionButtons={[
            {
              ...
              className: "px-4 py-2 bg-primary-dark text-white rounded-xl font-bold text-sm shadow-lg shadow-primary/20 hover:bg-primary-dark/90 active:scale-95 transition-transform flex items-center gap-2",
          ...
          {loading ? (
            ...
            <div className="mb-8 space-y-4 animate-slide-up">
              <div className="grid grid-cols-2 gap-4">
                <StatCard
                  label={t('inventory.stockValue', 'Stock Value')}
                  value={formatCurrency(inventory.reduce((sum, item) => sum + (Number(item.cost_price) * Number(item.quantity)), 0))}
                  color="blue"
                />
                <StatCard
                  label="Low Stock"
              ...
              if (editingItem) {
                ...
                if (newItem.cost_price > 0) {
                  ...
                  if (!transaction) {
                    ...
                    toast.success(`Expense transaction created: ${formatCurrency(newItem.cost_price * newItem.quantity)}`);
                  }
                } else {
