...
export default function Settings() {
  ...
  const { clearCountry } = useCountryStore();
  ...
  const loadUserData = async () => {
    try {
      // Use user data from auth store instead of querying database (RLS issue)
        ...
        <div className="px-4 mt-8 space-y-8">
          ...
          <div className="space-y-4 animate-slide-up" style={{ animationDelay: '0.2s' }}>
            ...
            <div className="bg-white rounded-[32px] border border-gray-50 shadow-sm overflow-hidden">
              ...
              <div className="settings-row-premium" onClick={() => navigate('/dashboard/settings/sessions')}>
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-500 flex items-center justify-center">
                    <Shield size={18} />
                  </div>
                  <span className="text-sm font-black text-gray-900">{t('settings.security', 'Security & Sessions')}</span>
                </div>
                <ChevronRight size={18} className="text-gray-300" />
              </div>
