...
export async function getCoachingContext(userId) {
  try {
    ...
    const currencySymbol = activeCountry?.currency?.symbol || 'KSh';
    const countryName = activeCountry?.name || 'Kenya';
...
export async function checkForProactiveInsights(userId) {
  try {
    ...
    if (today.getDay() === 1) { // Monday
      insights.push({
        type: 'weekly_summary',
        message: `Your week in numbers: R${context.current_month_profit.toFixed(0)} profit this month from ${context.transaction_count} transactions. ${context.trend === 'growing' ? 'Things are looking up! 📈' :
...
export function formatContextForPrompt(context, history = []) {
  ...
  const { activeCountry } = useCountryStore.getState();
  const symbol = activeCountry?.currency?.symbol || 'KSh';
