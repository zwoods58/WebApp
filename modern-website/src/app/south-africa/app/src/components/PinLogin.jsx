...
export default function PinLogin({ onLoginSuccess, phoneNumber, countryName, remoteAuthData, onForgotPin, onSwitchAccount }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const loginData = await AuthService.login(localPhone, activeCountry?.code || 'ZA', fullPin);
  ...
  return (
    <div className="bg-white/90 backdrop-blur-lg rounded-3xl shadow-2xl p-8 w-full border border-white/20">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600">Enter your 6-digit PIN to login</p>
      </div>
          ...
          <div className="flex justify-center gap-2 sm:gap-3">
            {pin.map((digit, index) => (
              <input
                ...
                onPaste={handlePaste}
                className="w-12 h-14 sm:w-14 sm:h-16 text-2xl sm:text-3xl font-bold text-center bg-gray-50 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-4 focus:ring-blue-100 outline-none transition-all duration-200 shadow-sm"
                maxLength={1}
                inputMode="numeric"
                pattern="[0-9]*"
                disabled={isLoading}
