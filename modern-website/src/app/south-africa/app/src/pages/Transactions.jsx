...
export default function Transactions() {
  ...
  const { transactions, deleteTransaction, loading } = useDemoData();
  ...
  const groupedTransactions = filteredTransactions.reduce((groups, transaction) => {
    const date = transaction.date ? transaction.date.split('T')[0] : new Date().toISOString().split('T')[0];
        ...
        <div className="px-4 mt-6">
          {loading ? (
            ...
            <div className="space-y-8 pb-10">
              {Object.entries(groupedTransactions).sort((a, b) => b[0].localeCompare(a[0])).map(([date, items]) => (
