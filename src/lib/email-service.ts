...
export class EmailService {
  ...
  constructor() {
    ...
    this.fromEmail = process.env.FROM_EMAIL || 'noreply@atarweb.com'
  }
  ...
  static getTemplates() {
    return {
      ...
      welcomeClient: (data: { name: string; company: string }): EmailTemplate => ({
        subject: 'Welcome to AtarWeb Solutions!',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <h2 style="color: #2563eb;">Welcome to AtarWeb Solutions!</h2>
            <p>Hi ${data.name},</p>
            <p>Welcome to AtarWeb Solutions! We're thrilled to have ${data.company} as our new client.</p>
